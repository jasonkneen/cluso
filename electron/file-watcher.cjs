...
// Reference to mgrep module (set lazily to avoid circular deps)
let mgrepModule = null
...
/**
 * Get the mgrep module (lazy load to avoid circular dependencies)
 */
function getMgrep() {
  if (!mgrepModule) {
    try {
      mgrepModule = require('./mgrep.cjs')
    } catch (err) {
      // mgrep not available
      console.log('[FileWatcher] mgrep module not available:', err.message)
    }
  }
  return mgrepModule
}
...
/**
 * Send file change event to renderer and notify LSP + mgrep
 */
function sendFileChange(type, filePath, projectPath) {
  ...
  // Notify mgrep of file changes for semantic indexing (async, don't block)
  const mgrep = getMgrep()
  if (mgrep) {
    // Map file watcher event types to mgrep event types
    const eventTypeMap = {
      'add': 'added',
      'change': 'modified',
      'unlink': 'deleted',
    ...
    const mgrepEvent = {
      filePath,
      eventType: eventTypeMap[type] || type,
      timestamp: Date.now(),
    }
    mgrep.onFileChange(mgrepEvent).catch(err => {
      console.log('[FileWatcher] mgrep file change failed:', err.message)
    })
  }
}
